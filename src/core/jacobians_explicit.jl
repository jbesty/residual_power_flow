...
@inline function _push_nonzero!(rows, cols, vals, row, col, val)
    if val != zero(val)
        push!(rows, row); push!(cols, col); push!(vals, val)
    end
end
...
function _build_voltage_jacobian_coo(ps, variables, controls, statuses,
    ...
    for ii in 1:n_single
        ...
        _push_nonzero!(rows, cols, vals, 2*bus-1, bus, s * dIre_dV)
        _push_nonzero!(rows, cols, vals, 2*bus,   bus, s * dIim_dV)
        ctrl_cursor += nc
    end
    ...
    for k in 1:n_branches
        ...
        _push_nonzero!(rows, cols, vals, row_re, from_bus, s * dIre_dVt)
        _push_nonzero!(rows, cols, vals, row_im, from_bus, s * dIim_dVt)
        _push_nonzero!(rows, cols, vals, row_re, to_bus,   s * dIre_dVo)
        _push_nonzero!(rows, cols, vals, row_im, to_bus,   s * dIim_dVo)
        _push_nonzero!(rows, cols, vals, row_re, col_θ,    s * dIre_dθ)
        _push_nonzero!(rows, cols, vals, row_im, col_θ,    s * dIim_dθ)
        ...
        _push_nonzero!(rows, cols, vals, row_re2, to_bus,   s * dIre_dVt2)
        _push_nonzero!(rows, cols, vals, row_im2, to_bus,   s * dIim_dVt2)
        _push_nonzero!(rows, cols, vals, row_re2, from_bus, s * dIre_dVo2)
        _push_nonzero!(rows, cols, vals, row_im2, from_bus, s * dIim_dVo2)
        _push_nonzero!(rows, cols, vals, row_re2, col_θ,    s * dIre_dθ2)
        _push_nonzero!(rows, cols, vals, row_im2, col_θ,    s * dIim_dθ2)
    end
    ...
    return sparse(rows, cols, vals, n_res, n_var)
end
...
function _fill_voltage_jacobian!(J::SparseMatrixCSC, ps, variables, controls, statuses,
    ...
    @inbounds for ii in 1:n_single
        ...
        J[2*bus-1, bus] += s * dIre_dV
        J[2*bus,   bus] += s * dIim_dV
        ctrl_cursor += nc
    end
    ...
    @inbounds for k in 1:n_branches
        ...
        dIre_dVt, dIim_dVt, dIre_dVo, dIim_dVo, dIre_dθ, dIim_dθ =
            _dI_dV_branch(br.component.Y_11, br.component.Y_12, V_to, θ, 1)
        J[2*from_bus-1, from_bus] += s * dIre_dVt
        J[2*from_bus,   from_bus] += s * dIim_dVt
        J[2*from_bus-1, to_bus]   += s * dIre_dVo
        J[2*from_bus,   to_bus]   += s * dIim_dVo
        J[2*from_bus-1, col_θ]    += s * dIre_dθ
        J[2*from_bus,   col_θ]    += s * dIim_dθ

        dIre_dVt2, dIim_dVt2, dIre_dVo2, dIim_dVo2, dIre_dθ2, dIim_dθ2 =
            _dI_dV_branch(br.component.Y_22, br.component.Y_21, V_from, θ, -1)
        J[2*to_bus-1, to_bus]   += s * dIre_dVt2
        J[2*to_bus,   to_bus]   += s * dIim_dVt2
        J[2*to_bus-1, from_bus] += s * dIre_dVo2
        J[2*to_bus,   from_bus] += s * dIim_dVo2
        J[2*to_bus-1, col_θ]    += s * dIre_dθ2
        J[2*to_bus,   col_θ]    += s * dIim_dθ2
    end
...
function _compute_jacobian_controls_impl(
    ...
    if _control_jacobian_cache !== nothing && _control_jacobian_cache[] !== nothing
        ...
        @inbounds for ii in 1:n_single
            ...
            for jj in 1:nc
                ...
                J[2*bus-1, global_col] += s * dI_du[1, jj]
                J[2*bus,   global_col] += s * dI_du[2, jj]
            end
            ctrl_cursor += nc
        end
        return J
    ...
    for ii in 1:n_single
        ...
        for jj in 1:nc
            global_col = ctrl_cursor + jj - 1
            _push_nonzero!(rows, cols, vals, row_re, global_col, s * dI_du[1, jj])
            _push_nonzero!(rows, cols, vals, row_im, global_col, s * dI_du[2, jj])
        end
        ctrl_cursor += nc
    end

    J = sparse(rows, cols, vals, n_res, n_ctrl)
