...
function compute_current_injection(
    ...
    complex_power = -complex(P, Q) * ZIP_load_factor
    i_complex = conj(complex_power / voltage)
...
# branch_angle = θ_to − θ_from, the branch angle variable φ (θ = bus angles).
...
function compute_current_injection(
    ...
    sin_branch_angle, cos_branch_angle = sincos(branch_angle)
    # From-end: local reference at from-bus (angle 0), to-bus at angle +branch_angle
    v_from             = complex(v_from_mag, zero(v_from_mag))
    v_to_from_frame    = v_to_mag * complex(cos_branch_angle, sin_branch_angle)
    i_from = -(component.Y_11 * v_from + component.Y_12 * v_to_from_frame)
    # To-end: local reference at to-bus (angle 0), from-bus at angle −branch_angle
    v_to               = complex(v_to_mag, zero(v_to_mag))
    v_from_to_frame    = v_from_mag * complex(cos_branch_angle, -sin_branch_angle)
