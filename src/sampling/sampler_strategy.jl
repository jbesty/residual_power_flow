...
#
#     _generate(::Strategy, ps, config, n; rng) -> (PowerFlowDataset, SamplerDiagnostics)
#
...
#
# `_strategy_for(config)` maps a config type to its strategy; the per-config methods
# live alongside each strategy. `_generate` is the seam each strategy implements.

generate_dataset_with_diagnostics(
    power_system::PowerSystem, config::AbstractSamplerConfig, n::Int;
    rng::AbstractRNG = default_rng(),
) = _generate(_strategy_for(config), power_system, config, n; rng)

generate_dataset(
    power_system::PowerSystem, config::AbstractSamplerConfig, n::Int;
    rng::AbstractRNG = default_rng(),
) = first(_generate(_strategy_for(config), power_system, config, n; rng))
