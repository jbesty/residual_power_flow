...
#
# config.optimiser is passed directly to Optimization.solve:
...
# Log-spaced iteration indices in [1, max_iterations] for energy recording: dense at the
# start (the first integers all appear) and thinning geometrically — captures the
# fast early L-BFGS descent while bounding the recorded points to ~n_points. Far cheaper
# than every-step on a log-axis convergence plot, where linear sampling wastes points.
function _energy_log_iters(max_iterations::Int, n_points::Int)
    (max_iterations <= 1 || n_points <= 1) && return Set(1:max(max_iterations, 1))
    return Set(round.(Int, exp.(range(0.0, log(max_iterations), length = n_points))))
end

function fit!(
    solver::NeuralSolver,
    datasets::Tuple{PowerFlowDataset,PowerFlowDataset},
    config::NeuralTraining;
    energy_every::Int = 1,
    ...
    energy_iters = energy_logpoints > 0 ?
        _energy_log_iters(config.max_iterations, energy_logpoints) : nothing
    ...
    step_cb! = function (flat_params, training_loss_value)
        ...
        if config.print_every > 0 && iter_count[] % config.print_every == 0
    ...
    objective = if config.optimiser isa Optim.FirstOrderOptimizer
        n_params = length(initial_flat_params)
        Optimization.OptimizationFunction(training_loss, Optimization.AutoZygote();
            hess = (H, θ) -> nothing,   # Optimization.jl keyword — do not rename
    ...
    backend_kwargs = if config.optimiser isa Optimisers.AbstractRule
        (; save_best = false)
    elseif isfinite(config.abstol)
...
function fit!(
    ...
    for config in phases
        _, d = fit!(solver, datasets, config; energy_every, energy_logpoints)
...
function fit!(
    ...
    isempty(config.candidates) && throw(ArgumentError("BestOf requires at least one candidate"))
    initial_params = deepcopy(solver.parameters)
    initial_states = solver.states

    n = length(config.candidates)
    ...
    for (i, phases) in enumerate(config.candidates)
        ...
        select_values[i] = _select_value(config.select_by, d)
    end
