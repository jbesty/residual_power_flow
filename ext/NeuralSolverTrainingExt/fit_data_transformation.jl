...
Schemes: `:identity`, `:standardised`, `:physics_informed`, chosen per side.
The paper's scheme comparison uses the same scheme on both sides; the default
here is standardised controls + physics-informed voltages.
"""
function fit_data_transformation(
    ...
    A_voltages = if voltages_scheme == :physics_informed
        # A_v = H_vv^{-1/2}: the paper's output relation A_y^T A_y = W_y^{-1} with W_y = H_yy.
    ...
    A_controls = if controls_scheme == :physics_informed
        ...
        # the backward-error magnitude (paper: W_x is the Schur complement
        # H_xy H_yy^{-1} H_yx of the residual-norm Hessian).
