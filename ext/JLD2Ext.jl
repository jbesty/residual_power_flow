...
function _signature_metadata(signature_data::DatasetSignature)
    return Dict{String,Any}(
        "has_signature"          => true,
        "signature/system_name"  => signature_data.system_name,
        "signature/n_controls"   => signature_data.n_controls,
        "signature/n_variables"  => signature_data.n_variables,
    )
end

function _write_metadata!(f, metadata_dict::AbstractDict)
...
function save_dataset(path::AbstractString, dataset::PowerFlowDataset; split::Symbol = :train)
    signature_data = DatasetSignature("unknown",
                           dataset.power_system.n_controls,
                           dataset.power_system.n_variables)
    metadata_dict = merge(_provenance_metadata(), _signature_metadata(signature_data))
    jldopen(path, "w") do f
        _write_metadata!(f, metadata_dict)
...
function save_dataset(
    ...
    metadata_dict = merge(_provenance_metadata(), Dict{String,Any}(string(k) => v for (k, v) in metadata))
    jldopen(path, "w") do f
        _write_metadata!(f, metadata_dict)
...
function save_config_dataset(
    ...
    signature_data = DatasetSignature(config.system_name,
    ...
    metadata = merge(
        ...
        _signature_metadata(signature_data),
    )
