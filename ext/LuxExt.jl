...
function _solver_to_portable(solver::NeuralSolver)
    ...
    return (format          = "neural-mlp/v1",
            ...
            A_controls       = Matrix{Float64}(t.A_controls),
            b_controls       = Vector{Float64}(t.b_controls),
            A_voltages       = Matrix{Float64}(t.A_voltages),
            b_voltages       = Vector{Float64}(t.b_voltages))
end

function _solver_from_portable(nt::NamedTuple)
    nt.format == "neural-mlp/v1" ||
        error("unsupported saved solver format $(repr(nt.format))")
    t = DataTransformation(nt.A_controls, nt.b_controls, nt.A_voltages, nt.b_voltages)
    solver = NeuralSolver(t;
