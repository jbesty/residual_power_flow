@testset "Current injections" begin
    ...
    @testset "Components" begin
        ...
        @testset "Branch" begin
            # New signature: (component, v_from_mag, v_to_mag, branch_angle)
            # Returns (i_from, i_to) where each is [i_d, i_q].
            # Each current is expressed in the local frame of its terminal bus:
            #   i_from: from-bus as reference (angle 0), to-bus at angle +branch_angle
            #   i_to:   to-bus as reference (angle 0), from-bus at angle −branch_angle
