# Load Revise before RPF so any edit to src/ or ext/ is hot-patched into the
# already-loaded module during interactive test runs. Wrapped in try/catch so a
# Revise load failure (e.g. a minimal env without the test deps) does not break
# the run.
try
    @eval import Revise
catch err
    @warn "Revise not loaded" err
end
...
# case9 fixture: an exact RPF fixed point (r ≈ 1e-14), deserialized from JLD2 so
