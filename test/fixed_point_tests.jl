...
if !isnothing(_CASE9)
    @testset "fixed point (A) — case9 Float64" begin
        ...
        # the way to machine zero — the fixed-point update is floored at ~‖r‖.
        # Solved from a flat start the JLD2 fixture floors at r ≈ 8e-9 (its
        # stored solution sits at r ≈ 5e-7); the slow-tier Powsybl build is
        # similar, around r ≈ 1e-8. Scale the threshold by ‖r‖ (the 1e-10 floor
        # rarely binds) so both work.
